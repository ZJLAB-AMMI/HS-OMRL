...
class SimpleReplayBuffer(ReplayBuffer):
    def __init__(
        ...
        self._trajectory_starts = np.zeros((max_replay_buffer_size, 1), dtype='uint8')
        self._policy_starts = np.zeros((max_replay_buffer_size, 1), dtype='uint8')
        self.clear()
    ...
    def add_sample(self, observation, action, reward, terminal,
        ...
        self._trajectory_starts[self._top] = new_trajectory
        self._policy_starts[self._top] = new_policy
        self._advance()
    ...
    def add_samples(self, observations, actions, rewards, terminals,
        ...
        if len(new_trajectories) == 1:
            new_trajectories = np.array(new_trajectories * n_samples)
        if len(new_policies) == 1:
            new_policies = np.array(new_policies * n_samples)
        self._trajectory_starts[self._top:self._top+n_samples] = new_trajectories
        self._policy_starts[self._top:self._top+n_samples] = new_policies
    ...
    def sample_data(self, indices):
        return dict(
            ...
            trajectory_starts=self._trajectory_starts[indices],
            policy_starts=self._policy_starts[indices],
        )
