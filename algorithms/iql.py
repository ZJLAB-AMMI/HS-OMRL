...
class IQL(nn.Module):
    def __init__(self,
        ...
        if self.encoder is not None:
            vf_params = self.vf.parameters() + self.encoder.parameters()
        else:
    ...
    def update(self, obs, action, reward, next_obs, done, **kwargs):
        ...
        with torch.no_grad():
            ...
            target_q1 = self.qf1_target(obs, action)
            target_q2 = self.qf2_target(obs, action)
        ...
        with torch.no_grad():
            ...
            target_q = target_q.detach().clone()
        current_q1 = self.qf1(obs, action)
        current_q2 = self.qf2(obs, action)
        ...
        _, mean, log_std, _ = self.act(obs)#, return_log_prob = True)
        # print(mean, log_std)
